...
class ConcentrationViewController: UIViewController {
    ...
    private var newGame: Concentration {
      return Concentration(numberOfPairsOfCards: (cardsCollection.count + 1 ) / 2, theme: theme)
    }
  
    var theme: GameTheme.CurrentTheme? {
      didSet {
        if cardsCollection != nil { startNewGame(UIButton()) }
      }
    }
  
    private var gameAttributes: [NSAttributedString.Key: Any] {
    ...
    private func updateViewFromModel() {
      ...
      private func animated(Button button: UIButton,
                            color: UIColor,
                            title: String,
                            optional: UIView.AnimationOptions,
